...
namespace Dungeon12.SceneObjects.Base
{
    ...
    {
        public GameHint(string title, string text, AbilityArea area = null, int cooldown = -1, params string[] leftparams) : base(title, text, area, cooldown, leftparams)
        {
        }
    }

    internal class ObjectPanel : EmptySceneObject, ISceneObjectHosted
    {
        public ObjectPanel(string title, string text, AbilityArea area = default, int cooldown = -1, params string[] leftparams)
        {
            this.Width = 355;
            ...
            var ttle = title.AsDrawText().Gabriela().InColor(Global.CommonColorLight).InSize(25);
