...
using Dungeon.Monogame.Monogame;
...
namespace Dungeon.Monogame
{
    ...
    {
        public static MonoGameClient Instance;

        FPSMeter FPS = new FPSMeter();

        public MonoGameClient(MonogameSettings settings)
        {
            try
            {
                SDL_InitMonitors();
...
#warning penumbra need to change to monogame.extended
            //LoadPenumbra();
            Load3D();
            ...
            this.Content = new DungeonContentManager(Services,SceneManager);
            SceneManager.Start(isFatal ? "FATAL" : default);
        }
